...
export const Marker = ({ map, country, count, zoom }: MarkerProps) => {
  ...
  useEffect(() => {
    ...
    // Store reference to country text element
  ...
  // Separate effect to handle zoom-based visibility changes
  useEffect(() => {
    if (!countryTextRef.current || zoom === undefined) return;

    // Calculate radius (you'll need to copy the interpolate function logic here)
    const radiusStops = [
      { count: 1, radius: 10 },
      { count: 5, radius: 15 },
      { count: 20, radius: 25 },
  ...
  }, [zoom, count]); // Only zoom and count
